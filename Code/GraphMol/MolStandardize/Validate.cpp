...
std::vector<ValidationErrorInfo> DisallowedRadicalValidation::validate(
  ...
  for (auto atom: mol.atoms()) {
    ...
    errors.push_back(
      "ERROR: [DisallowedRadicalValidation] The radical at atom " + std::to_string(atom->getIdx()) + " is not allowed");
    if (!reportAllFailures) {
      break;
    }
  }
...
std::vector<ValidationErrorInfo> Is2DValidation::validate(
  ...
  if (!mol.getNumConformers()) {
    errors.emplace_back("ERROR: [Is2DValidation] The molecule has no coordinates");
    return errors;
  }
  ...
  if (max_absz > threshold) {
    errors.emplace_back("ERROR: [Is2DValidation] The molecule includes non-null Z coordinates");
    if (!reportAllFailures) {
      return errors;
    }
  }
...
std::vector<ValidationErrorInfo> Layout2DValidation::validate(
  ...
  if (!mol.getNumConformers()) {
    errors.emplace_back("ERROR: [Layout2DValidation] The molecule has no coordinates");
    return errors;
  }
  ...
  for (unsigned int i = 0; i < natoms - 1; ++i) {
    ...
    for (unsigned int j = i + 1; j < natoms; ++j) {
      ...
      if (d2 < atomClashThreshold) {
        errors.push_back(
          "ERROR: [Layout2DValidation] Atom " + std::to_string(i)
          + " is too close to atom " + std::to_string(j));
        if (!reportAllFailures) {
          return errors;
        }
      }
  ...
  for (auto bond : mol.bonds()) {
    ...
    if (!allowLongBondsInRings || mol.getRingInfo()->numBondRings(bond->getIdx()) == 0) {
      ...
      if (ll > bondLengthThreshold) {
        errors.push_back(
          ...
          + " between atoms " + std::to_string(i) + " and " + std::to_string(j)
    ...
    for (unsigned int k = 0; k < natoms; ++k) {
      ...
      if (
        ...
        errors.push_back(
          "ERROR: [Layout2DValidation] Atom " + std::to_string(k)
          + " too close to bond " + std::to_string(bond->getIdx()));
        if (!reportAllFailures) {
          return errors;
        }
      }
        ...
        if (bondDir == Bond::BondDir::BEGINDASH || bondDir == Bond::BondDir::BEGINWEDGE) {
          ...
          if (angle < 185.*M_PI/180.) {
            errors.push_back(
              ...
              "non-stereo bonds at atom " + std::to_string(atom->getIdx()) /* +
